...
execute store result score %rand.type repo_dummy run random value 1..174
...
execute if score %rand.type repo_dummy matches 53..93 run data modify storage repo:rand type set value "minecraft:parrot"
execute if score %rand.type repo_dummy matches 94..174 run data modify storage repo:rand type set value "minecraft:panda"

tellraw @a[tag=repo.debug] [{"text": "Detected Bamboo Jungle","color": "gray"}]
