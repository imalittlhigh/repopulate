...
execute store result score %rand.type repo_dummy run random value 1..48
...
execute if score %rand.type repo_dummy matches 31..42 run data modify storage repo:rand type set value "minecraft:sheep"
execute if score %rand.type repo_dummy matches 43..48 run data modify storage repo:rand type set value "minecraft:wolf"

tellraw @a[tag=repo.debug] [{"text": "Detected Forest","color": "gray"}]
