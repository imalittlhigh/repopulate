...
execute store result score %rand.type repo_dummy run random value 1..56
...
execute if score %rand.type repo_dummy matches 44..51 run data modify storage repo:rand type set value "minecraft:wolf"
execute if score %rand.type repo_dummy matches 52..56 run data modify storage repo:rand type set value "minecraft:rabbit"

tellraw @a[tag=repo.debug] [{"text": "Detected Grove","color": "gray"}]
