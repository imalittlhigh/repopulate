...
execute store result score %rand.type repo_dummy run random value 1..54
...
execute if score %rand.type repo_dummy matches 44..54 run data modify storage repo:rand type set value "minecraft:frog"

tellraw @a[tag=repo.debug] [{"text": "Detected Mangrove Swamp","color": "gray"}]
