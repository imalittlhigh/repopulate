...
execute store result score %rand.type repo_dummy run random value 1..42
...
execute if score %rand.type repo_dummy matches 19..29 run data modify storage repo:rand type set value "minecraft:pig"
execute if score %rand.type repo_dummy matches 30..42 run data modify storage repo:rand type set value "minecraft:sheep"

tellraw @a[tag=repo.debug] [{"text": "Detected Old Growth Birch Forest","color": "gray"}]
