...
execute store result score %rand.type repo_dummy run random value 1..65
...
execute if score %rand.type repo_dummy matches 44..52 run data modify storage repo:rand type set value "minecraft:wolf"
execute if score %rand.type repo_dummy matches 53..61 run data modify storage repo:rand type set value "minecraft:fox"
execute if score %rand.type repo_dummy matches 62..65 run data modify storage repo:rand type set value "minecraft:rabbit"

tellraw @a[tag=repo.debug] [{"text": "Detected Old Growth Spruce Taiga","color": "gray"}]
