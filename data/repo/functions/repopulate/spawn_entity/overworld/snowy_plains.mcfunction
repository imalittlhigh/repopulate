...
execute store result score %rand.type repo_dummy run random value 1..2
...
execute if score %rand.type repo_dummy matches 2 run data modify storage repo:rand type set value "minecraft:polar_bear"

tellraw @a[tag=repo.debug] [{"text": "Detected Snowy Plains","color": "gray"}]
