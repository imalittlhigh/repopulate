...
execute store result score %rand.type repo_dummy run random value 1..10
...
execute if score %rand.type repo_dummy matches 1..4 run data modify storage repo:rand type set value "minecraft:rabbit"
execute if score %rand.type repo_dummy matches 5..10 run data modify storage repo:rand type set value "minecraft:goat"

tellraw @a[tag=repo.debug] [{"text": "Detected Snowy Slopes","color": "gray"}]
