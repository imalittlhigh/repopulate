...
execute store result score %rand.type repo_dummy run random value 1..49
...
execute if score %rand.type repo_dummy matches 44..49 run data modify storage repo:rand type set value "minecraft:llama"

tellraw @a[tag=repo.debug] [{"text": "Detected Windswept Forest","color": "gray"}]
