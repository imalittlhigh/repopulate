...
execute store result score %rand.type repo_dummy run random value 1..45
...
execute if score %rand.type repo_dummy matches 44 run data modify storage repo:rand type set value "minecraft:horse"
execute if score %rand.type repo_dummy matches 45 run data modify storage repo:rand type set value "minecraft:donkey"

tellraw @a[tag=repo.debug] [{"text": "Detected Windswept Savanna","color": "gray"}]
